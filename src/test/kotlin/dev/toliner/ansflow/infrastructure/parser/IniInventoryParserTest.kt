    ...
    Given("IniInventoryParser") {
        ...
        When("parsing simple inventory") {
            ...
            Then("should parse groups and hosts correctly") {
                ...
                webGroup.hosts.map { it.name } shouldContainExactlyInAnyOrder listOf(
                    "web1.example.com",
                    "web2.example.com"
                )
                ...
                dbGroup.hosts.map { it.name } shouldContainExactlyInAnyOrder listOf(
                    "db1.example.com",
                    "db2.example.com"
                )
            }
        ...
        When("parsing inventory with group variables") {
            ...
            Then("should apply group variables to all hosts in group") {
                ...
                webGroup.hosts.forEach { host ->
                    host.variables shouldContainExactly mapOf(
        ...
        When("parsing inventory with nested groups") {
            ...
            Then("should create hierarchical group structure") {
                ...
                val productionGroup = inventory.findGroup("production")!!
        ...
        When("parsing complex inventory") {
            ...
            Then("should handle all features together") {
                ...
                val web1 = inventory.findGroup("web")!!.hosts.find { it.name == "web1.example.com" }!!
                web1.variables shouldContainExactly mapOf(
                    "ansible_host" to "192.168.1.10",
                    "http_port" to "80",
        ...
        When("handling partial success scenarios") {
            ...
            Then("should parse valid sections and report errors") {
                ...
                // Parser can choose to fail entirely or parse what it can
                // For robustness, we'll implement partial parsing
                result.shouldBeSuccess()
                val inventory = result.getOrNull()!!
                
                // Should have parsed web and db groups
                inventory.groups shouldHaveSize 2
                inventory.findGroup("web").shouldNotBeNull()
                inventory.findGroup("db").shouldNotBeNull()
                inventory.findGroup("invalid") shouldBe null
            }
        }
